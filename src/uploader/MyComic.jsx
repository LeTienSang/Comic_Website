...
const MyComic = () => { // ✅ Không dùng async ở đây
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error('Error fetching comics:', err.response?.data || err.message);
      }
    };
  ...
  useEffect(() => {
    console.log('Comics updated:', comics);
  }, [comics]);
      ...
      {comics.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {comics.map((comic) => (
            ...
            >
              <img
                src={comic.cover_image || '/placeholder.jpg'}
                alt={comic.title}
